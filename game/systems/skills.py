...
def use_skill_reveal(user_id, puzzle_data):
    ...
    # Memeriksa apakah pemain memiliki 'Wisdom Tome' di daftar artefak permanen
    artifacts = player.get('artifacts', [])
    has_wisdom_tome = 'wisdom_tome' in artifacts
    ...
    if current_mp < cost:
        return False, f"🔮 Fokusmu memudar... (Butuh {cost} MP, MP saat ini: {int(current_mp)})", puzzle_data.get('current_hint', '')
    ...
    if not current_hint or current_hint == "":
    ...
    # Pastikan mengambil karakter asli dari jawaban
    new_hint_list[idx_to_reveal] = answer[idx_to_reveal]
    new_hint = "".join(new_hint_list)

    # 6. Update Database
    update_player(user_id, {"mp": max(0, current_mp - cost)})
...
def use_skill_freeze(user_id, current_timer):
    """
    Logika Skill Temporal Freeze: Menambahkan waktu ke timer puzzle.
    Biaya: 15 MP.
    """
    player = get_player(user_id)
    cost = 15
    ...
    current_mp = player.get('mp', 0)
    if current_mp < cost:
        return False, f"🔮 Mana tidak cukup! (Butuh {cost} MP)", current_timer
        
    # Tambahkan 15 detik ke timer
    new_timer = current_timer + 15
    update_player(user_id, {"mp": current_mp - cost})
    
    return True, "❄️ **Temporal Freeze!** Waktu melambat untukmu. (+15s)", new_timer
